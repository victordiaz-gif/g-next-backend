...
async function importData() {
    console.log('🚀 Starting 100K products optimized import...');
    ...
    try {
        ...
        await processCSVOptimized(csvPath, importer, ctx);
        
        console.log('🔄 Rebuilding search index...');
        await searchService.reindex(ctx);
        
        console.log('✅ 100K products optimized import completed successfully!');
...
async function processCSVOptimized(csvPath: string, importer: Importer, ctx: RequestContext) {
    return new Promise<void>((resolve, reject) => {
        let headers: string[] = [];
        let lineNumber = 0;
        let currentProduct: ParsedProductWithVariants | null = null;
        let batch: ParsedProductWithVariants[] = [];
        const batchSize = 50; // Very small batch size
