...
async function importData() {
    console.log('🚀 Starting 100K products line-by-line import...');
    ...
    try {
        ...
        await processCSVLineByLine(csvPath, importer, ctx);
        
        console.log('🔄 Rebuilding search index...');
        await searchService.reindex(ctx);
        
        console.log('✅ 100K products line-by-line import completed successfully!');
...
async function processCSVLineByLine(csvPath: string, importer: Importer, ctx: RequestContext) {
    return new Promise<void>((resolve, reject) => {
        let headers: string[] = [];
        let lineNumber = 0;
        let currentProduct: ParsedProductWithVariants | null = null;
        let batch: ParsedProductWithVariants[] = [];
        const batchSize = 100; // Small batch size to avoid memory issues
